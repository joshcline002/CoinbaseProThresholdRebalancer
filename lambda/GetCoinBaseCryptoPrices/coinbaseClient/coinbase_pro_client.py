...
def get_product_ticker_price(private_client, product_id):
    ticker = private_client.get_product_ticker(product_id)
    if ticker == 'NotFound':
        ticker = None
...
def place_order(private_client, product_id, side, funds):
    place_order = private_client.place_order(product_id=product_id, side=side, order_type='market',
                                             funds=funds)
    return place_order
