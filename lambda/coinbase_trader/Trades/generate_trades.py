from trader_config.config import REBALANCED_TOLERANCE, FEE_PERCENT
...
def generate_trade(crypto, quote_currency, portfolio_value, quote_currency_amount):
    ...
    if target_percent != 0:
        ...
        if not abs(change_vs_portfolio) >= REBALANCED_TOLERANCE:
            return None, quote_currency_amount
        quote_amount_needed, shares, side = crypto_outside_tolerance(change_vs_portfolio,
                                                                     crypto, portfolio_value,
                                                                     quote_currency,
...
def crypto_outside_tolerance(change_vs_portfolio, crypto, portfolio_value,
                             quote_currency, quote_currency_amount, target_percent):
    ...
    if side == 'buy' and quote_amount_needed > quote_currency_amount:
        quote_amount_needed = quote_currency_amount
    return quote_amount_needed, shares, side
